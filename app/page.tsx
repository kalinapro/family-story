...
import { analyzePhotoFaces, supportsLocalFaceDetection } from "@/lib/face-analysis";
import { clusterFaceDetections, detachFace, isPhotoInCurrentStory, makePeople, mergePeople, photosWithoutPeople, type FaceDetection, type Person } from "@/lib/people";

type Photo = { id: string; file: File; fileName: string; url: string; takenAt: Date | null; dateTimeOriginal: Date | null; createDate: Date | null; selectedForStory: boolean; manualExcluded: boolean; hash?: string; perceptual?: Awaited<ReturnType<typeof createPerceptualImage>>; perceptualError?: string; faceIds: string[]; qualityScore: number | null; similarityGroupId: string | null };
type FamilyEvent = { id: string; title: string; photos: Photo[]; start: Date; end: Date };
type View = "all" | "story" | "timeline" | "excluded" | "duplicates" | "similar" | "people" | "faceless";
...
export default function Home() {
  ...
  const storyPhotos = useMemo(() => photos.filter((photo) => isPhotoInCurrentStory(photo, selectedPersonIds, people)), [photos, selectedPersonIds, people]);
  const displayed = useMemo(() => sortPhotos(view === "excluded" ? photos.filter((p) => p.manualExcluded || !p.selectedForStory) : view === "story" ? storyPhotos : view === "faceless" ? photosWithoutPeople(photos) : photos, sort), [photos, storyPhotos, sort, view]);
  ...
  async function addFiles(list: FileList | File[]) {
    ...
    const loaded = await Promise.all(valid.map(async (file): Promise<Photo> => {
      ...
      return { id: crypto.randomUUID(), file, fileName: file.name, url: URL.createObjectURL(file), takenAt, dateTimeOriginal, createDate, selectedForStory: true, manualExcluded: false, hash, perceptual, perceptualError, faceIds: [], qualityScore, similarityGroupId: null };
    }));
  ...
  function updateParticipation(id: string, selected: boolean) { setPhotos((current) => { const next = setStoryParticipation(current, id, selected).map((photo) => photo.id === id ? { ...photo, manualExcluded: !selected } : photo); setEvents(makeEvents(next)); return next; }); }
  ...
  async function findPeople() {
    setFaceError(""); setFaceProgress({ done: 0, total: photos.length }); const detected: FaceDetection[] = [];
    if (!supportsLocalFaceDetection()) { setFaceError("Этот браузер не поддерживает локальный FaceDetector. Откройте приложение в Chrome/Edge с поддержкой Shape Detection API."); setFaceProgress(null); return; }
    for (let index = 0; index < photos.length; index += 1) {
      try { detected.push(...await analyzePhotoFaces(photos[index].file, photos[index].id)); } catch { /* an unreadable photo stays faceless */ }
      setFaceProgress({ done: index + 1, total: photos.length }); await new Promise<void>((resolve) => setTimeout(resolve, 0));
    }
    const nextPeople = makePeople(clusterFaceDetections(detected)); setFaces(detected); setPeople(nextPeople);
    const byPhoto = new Map<string, string[]>(); detected.forEach((face) => byPhoto.set(face.photoId, [...(byPhoto.get(face.photoId) ?? []), face.id]));
    setPhotos((current) => current.map((photo) => ({ ...photo, faceIds: byPhoto.get(photo.id) ?? [] }))); setFaceProgress(null); setView("people");
  }
  function mergeWith(personId: string, sourceId: string) { setPeople((current) => mergePeople(current, personId, sourceId)); setSelectedPersonIds((current) => current.filter((id) => id !== sourceId)); }
  function togglePerson(id: string) { setSelectedPersonIds((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id]); }

  const visibleEvents = useMemo(() => events.flatMap((event) => { const included = event.photos.filter((photo) => isPhotoInCurrentStory(photo, selectedPersonIds, people)); return included.length ? [{ ...event, ...bounds(included) }] : []; }), [events, selectedPersonIds, people]);
  const openIndex = events.findIndex((event) => event.id === openEventId);
  const rawOpenEvent = openIndex >= 0 ? events[openIndex] : null;
  const openPhotos = rawOpenEvent?.photos.filter((photo) => isPhotoInCurrentStory(photo, selectedPersonIds, people)) ?? [];
  const openEvent = rawOpenEvent && openPhotos.length ? { ...rawOpenEvent, ...bounds(openPhotos) } : null;
  ...
  return <main><header className="topbar"><a className="brand" href="#top"><span className="brand-mark">С</span><span>Семейная история</span></a><span className="privacy"><span className="privacy-dot" />Фото и их цифровые отпечатки остаются на устройстве</span></header>
    ...
    {photos.length > 0 && <section className="collection"><div className="view-switch"><button className={view === "all" ? "active" : ""} onClick={() => { setView("all"); setOpenEventId(null); }}>Все фотографии</button><button className={view === "story" ? "active" : ""} onClick={() => setView("story")}>Текущая история <b>{storyPhotos.length}</b></button><button className={view === "people" ? "active" : ""} onClick={() => setView("people")}>Люди <b>{people.length}</b></button><button className={view === "timeline" ? "active" : ""} onClick={() => setView("timeline")}>Хронология</button><button className={view === "excluded" ? "active" : ""} onClick={() => setView("excluded")}>Исключённые <b>{stats.excluded}</b></button><button className={view === "duplicates" ? "active" : ""} onClick={() => setView("duplicates")}>Точные дубли <b>{stats.duplicateGroups}</b></button><button className={view === "similar" ? "active" : ""} onClick={() => setView("similar")}>Похожие кадры <b>{similarPhotoCount}</b></button><button className={view === "faceless" ? "active" : ""} onClick={() => setView("faceless")}>Без людей <b>{photosWithoutPeople(photos).length}</b></button></div>
      ...
      {(view === "all" || view === "story" || view === "excluded" || view === "faceless") && <><div className="collection-heading"><div><span className="section-kicker">{view === "excluded" ? "НЕ УЧАСТВУЮТ В ИСТОРИИ" : view === "story" ? "ФИЛЬТР ПО ПЕРСОНАЖАМ" : view === "faceless" ? "АТМОСФЕРНЫЕ КАДРЫ" : "ВАША КОЛЛЕКЦИЯ"}</span><h2>{view === "excluded" ? "Исключённые фотографии" : view === "story" ? "Текущая история" : view === "faceless" ? "Без распознанных людей" : "Семейные моменты"}</h2></div><div className="actions"><label>Сортировка <select value={sort} onChange={(e) => setSort(e.target.value as PhotoSort)}><option value="oldest">По дате: сначала старые</option><option value="newest">По дате: сначала новые</option><option value="name">По имени файла</option></select></label><button className="clear" onClick={clearAll}>Очистить всё</button></div></div>{displayed.length ? <div className="photo-grid">{displayed.map((photo) => photoCard(photo))}</div> : <div className="empty-state">Здесь пока нет фотографий.</div>}</>}
      {view === "people" && <section className="people-panel"><div className="collection-heading"><div><span className="section-kicker">ЛОКАЛЬНЫЙ АНАЛИЗ</span><h2>Люди</h2></div><button className="primary-button" disabled={Boolean(faceProgress)} onClick={() => void findPeople()}>{faceProgress ? "Анализируем лица…" : "Найти людей"}</button></div><div className="privacy-card"><strong>Ваши воспоминания остаются вашими</strong><p>Распознавание лиц выполняется на вашем устройстве. Фотографии и данные лиц не отправляются на сервер.</p>{faceProgress && <progress max={faceProgress.total} value={faceProgress.done} />} {faceProgress && <span>Обработано {faceProgress.done} из {faceProgress.total} фотографий</span>}{faceError && <p className="face-error">{faceError}</p>}</div>{people.length > 0 && <><div className="story-picker"><span className="section-kicker">СЛЕДУЮЩИЙ ШАГ</span><h2>Про кого эта история?</h2><p>Выберите одного или нескольких людей. Подходит фото, где есть хотя бы один из них.</p><div className="person-checks">{people.map((person,index)=><label key={person.id}><input type="checkbox" checked={selectedPersonIds.includes(person.id)} onChange={()=>togglePerson(person.id)} /> {person.name || `Человек ${index+1}`}</label>)}</div></div><div className="people-grid">{people.map((person,index)=><article className="person-card" key={person.id}><div className="portrait-stack">{person.representativePhotoIds.map(id=>photos.find(photo=>photo.id===id)).filter(Boolean).map(photo=>/* eslint-disable-next-line @next/next/no-img-element */<img key={photo!.id} src={photo!.url} alt="" />)}</div><h3>{person.name || `Человек ${index+1}`}</h3><p>{person.photoIds.length} фото · {person.faceDetectionIds.length} лиц</p><label>Кто это?<input value={person.name} placeholder="Например, Мама" onChange={event=>setPeople(current=>current.map(item=>item.id===person.id?{...item,name:event.target.value}:item))} /></label><label>Это один человек с:<select defaultValue="" onChange={event=>{if(event.target.value)mergeWith(person.id,event.target.value)}}><option value="">Выберите группу</option>{people.filter(item=>item.id!==person.id).map((item,itemIndex)=><option key={item.id} value={item.id}>{item.name||`Человек ${itemIndex+1}`}</option>)}</select></label><details><summary>Исправить ошибку в группе</summary>{person.faceDetectionIds.map(faceId=>{const face=faces.find(item=>item.id===faceId), photo=photos.find(item=>item.id===face?.photoId); return face&&photo?<button key={faceId} onClick={()=>setPeople(current=>detachFace(current,person.id,face))}>«{photo.fileName}»: это другой человек</button>:null})}</details></article>)}</div></>}</section>}
